...
func printStats(objs *bpfObjects) {
	...
	for iter.Next(&key, &value) {
		...
		// EXERCISE 3: uncomment to delete PIDs that only executed once.
		// if value == 1 {
		// 	if err := objs.PidCounts.Delete(&key); err != nil {
		// 		log.Printf("    ❌ Error deleting PID %d: %v", key, err)
		// 	} else {
		// 		log.Printf("    🗑️  Deleted PID %d (single execution)", key)
		// 	}
		// }
	}
