...
func main() {
	...
	// EXERCISE 4: Attach TCX ingress program
	...
	// Egress attach mirrors the ingress attach above, using the egress program
	// and hook. Pre-filled so you can focus on the ingress path (Exercise 5).
	linkEgress, err := link.AttachTCX(link.TCXOptions{
		Interface: iface.Index,
		Program:   objs.TrackEgressMacs,
	...
	for range ticker.C {
		...
		// EXERCISE 5: Iterate through mac_tracker map
		iter := /* YOUR CODE HERE */
		count := 0

		for iter.Next(&macKey, &info) {
			// Convert the u64 key back to a readable MAC via the u64ToMac()
			// helper below (pre-filled - it's the inverse of mac_to_u64 in the C).
			macStr := u64ToMac(macKey)
